...
mesh_npix, mesh_ie = np.meshgrid(arr_npix, arr_ie) # each output array (NxN shaped) contains x or y value at given (i,j)-th position
mesh = np.stack((mesh_npix, mesh_ie), axis=-1)

NPIX = len(arr_npix)
Nie = len(arr_ie)
arr_inputs = mesh.reshape(NPIX*Nie,2) # flatten to 2D array

# inputs are saved as integers
...
gen_command = ( 'python '+'/het/p4/'+username+
               '/gcewavelets/skysearch/code/generate_patches.py')
...
jdlfile.write("Arguments = $(npix) $(ie)\n")
...
jdlfile.write("Output = /het/p4/"+username+"/gcewavelets/skysearch/condor/"+runname+'$(npix)'+'_'+'$(ie)'+".out\n")
jdlfile.write("Error = /het/p4/"+username+"/gcewavelets/skysearch/condor/"+runname+'$(npix)'+'_'+'$(ie)'+".err\n")
...
jdlfile.write("queue npix,ie from inputs_list\n") # data file should not have format
jdlfile.close()
